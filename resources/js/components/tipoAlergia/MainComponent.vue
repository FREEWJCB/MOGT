      ...
      <div class="container bg-white">
        <div class="row justify-content-between">
          <div class="col-5 mt-2">
            <h2>List of Type Allergy</h2>
          </div>
          <div class="col-3 mt-2">
            <small>
              <span class="fas fa-home"></span> Maestro <span class="text-secondary">> Tipo de Alergia</span>
            </small>
          </div>
        </div>
        <div class="row justify-content-between">
...
export default {
  ...
  methods: {
    ...mapActions('tipoAlergia',['getAllTipoA']),
    ...mapMutations('tipoAlergia', ['cleanAllTipoA']),
    count(){
      axios.get('/tipoAlergia/contar')
      .then((value) => {
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    next(this.getAllTipoA({ pag: to.params.pag, buscar: ''}));
  },
  beforeRouteLeave (to, from, next) {
    const answer = window.confirm('Do you really want to leave?')
    if (answer) {
      next(this.cleanAllTipoA())
    } else {
      next(false)
    }
  },
  ...
  mounted() {
    ...
    this.getAllTipoA({ pag: this.$route.params.pag, buscar: ''});
    this.count();
  },
}
</script>
