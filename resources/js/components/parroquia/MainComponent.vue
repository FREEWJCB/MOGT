      ...
      <div class="container bg-white">
        <div class="row justify-content-between">
          <div class="col-5 mt-2">
            <h2>List of Parroquia</h2>
          </div>
          <div class="col-3 mt-2">
            <small>
              <span class="fas fa-home"></span> Maestro <span class="text-secondary">> Parroquia</span>
            </small>
          </div>
        </div>
        <div class="row justify-content-between">
...
export default {
  ...
  methods: {
    ...mapActions('parroquia',['getAllParroquias']),
    ...mapMutations('parroquia', ['cleanAllParroquia']),
    count(){
      axios.get('/parroquia/contar')
      .then((value) => {
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    next(this.getAllParroquias({ pag: to.params.pag, buscar: ''}));
  },
  beforeRouteLeave (to, from, next) {
    const answer = window.confirm('Do you really want to leave?')
    if (answer) {
      next(this.cleanAllParroquia())
    } else {
      next(false)
    }
  },
  ...
  mounted() {
    ...
    this.getAllParroquias({ pag: this.$route.params.pag, buscar: ''});
    this.count();
  },
}
</script>
