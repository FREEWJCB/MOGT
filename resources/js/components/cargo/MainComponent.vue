      ...
      <div class="container bg-white">
        <div class="row justify-content-between">
          <div class="col-5 mt-2">
            <h2>List of Type Cargo</h2>
          </div>
          <div class="col-3 mt-2">
            <small>
              <span class="fas fa-home"></span> Maestro <span class="text-secondary">> Cargo</span>
            </small>
          </div>
        </div>
        <div class="row justify-content-between">
...
export default {
  ...
  methods: {
    ...mapActions('cargo',['getAllCargo']),
    ...mapMutations('cargo', ['cleanAllCargo']),
    count(){
      axios.get('/cargo/contar')
      .then((value) => {
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    next(this.getAllCargo({ pag: to.params.pag, buscar: ''}));
  },
  beforeRouteLeave (to, from, next) {
    const answer = window.confirm('Do you really want to leave?')
    if (answer) {
      next(this.cleanAllCargo())
    } else {
      next(false)
    }
  },
  ...
  mounted() {
    ...
    this.getAllCargo({ pag: this.$route.params.pag, buscar: ''});
    this.count();
  },
}
</script>
