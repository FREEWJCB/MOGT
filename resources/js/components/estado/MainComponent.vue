      ...
      <div class="container bg-white">
        <div class="row justify-content-between">
          ...
          <div class="col-3 mt-2">
            <small>
              <span class="fas fa-home"></span> Maestro <span class="text-secondary">> Estado</span>
            </small>
          </div>
        </div>
        <div class="row justify-content-between">
...
export default {
  ...
  methods: {
    ...mapActions('estado',['getAllEstado']),
    ...mapMutations('estado', ['cleanAllEstado']),
    count(){
      axios.get('/estado/contar')
      .then((value) => {
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    next(this.getAllEstado({ pag: to.params.pag, buscar: ''}));
  },
  beforeRouteLeave (to, from, next) {
    const answer = window.confirm('Do you really want to leave?')
    if (answer) {
      next(this.cleanAllEstado())
    } else {
      next(false)
    }
  },
  ...
  mounted() {
    ...
    this.getAllEstado({ pag: this.$route.params.pag, buscar: ''});
    this.count();
  },
}
</script>
