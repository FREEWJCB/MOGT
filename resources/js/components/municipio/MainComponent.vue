      ...
      <div class="container bg-white">
        <div class="row justify-content-between">
          <div class="col-5 mt-2">
            <h2>List of Municipaly</h2>
          </div>
          <div class="col-3 mt-2">
            <small>
              <span class="fas fa-home"></span> Maestro <span class="text-secondary">> Municipio</span>
            </small>
          </div>
        </div>
        <div class="row justify-content-between">
...
export default {
  ...
  methods: {
    ...mapActions('municipio',['getAllMunicipios']),
    ...mapMutations('municipio', ['cleanAllMunicipio']),
    count(){
      axios.get('/municipio/contar')
      .then((value) => {
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    next(this.getAllMunicipios({ pag: to.params.pag, buscar: ''}));
  },
  beforeRouteLeave (to, from, next) {
    const answer = window.confirm('Do you really want to leave?')
    if (answer) {
      next(this.cleanAllMunicipio())
    } else {
      next(false)
    }
  },
  ...
  mounted() {
    ...
    this.getAllMunicipios({ pag: this.$route.params.pag, buscar: ''});
    this.count();
  },
}
</script>
