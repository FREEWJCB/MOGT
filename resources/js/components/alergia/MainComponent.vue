      ...
      <div class="container bg-white">
        <div class="row justify-content-between">
          <div class="col-5 mt-2">
            <h2>List of Allergy</h2>
          </div>
          <div class="col-3 mt-2">
            <small>
              <span class="fas fa-home"></span> Maestro <span class="text-secondary">> Alergia</span>
            </small>
          </div>
        </div>
        <div class="row justify-content-between">
...
export default {
  ...
  methods: {
    ...mapActions('alergia',['getAllAlergias']),
    ...mapMutations('alergia', ['cleanAllAlergia']),
    count(){
      axios.get('/alergia/contar')
      .then((value) => {
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    next(this.getAllAlergias({ pag: to.params.pag, buscar: ''}));
  },
  beforeRouteLeave (to, from, next) {
    const answer = window.confirm('Do you really want to leave?')
    if (answer) {
      next(this.cleanAllAlergia())
    } else {
      next(false)
    }
  },
  ...
  mounted() {
    ...
    this.getAllAlergias({ pag: this.$route.params.pag, buscar: ''});
    this.count();
  },
}
</script>
